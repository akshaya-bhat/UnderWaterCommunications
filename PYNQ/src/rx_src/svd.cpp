...
void svd(float UU[], float S[], float VV[], float X[], int N, int M)
{
	...
	float *V = VV, *U = UU;
	std::cout << "svd entered" << std::endl;
	if (N < M) { //transpose if rows are less than columns
		std::cout << "svd 1" << std::endl;
		int tmp = N;
		float *Xnew;
		Xnew = new float[M*N];
...
void psInv(float Y[], float X[], int N, int M)
{
	...
	std::cout << "psInv start" << std::endl;
	float *U, *V, *S, *Sinv, *Utr, *V_Sinv;
	bool transposed = false;
	if (N < M) { //transpose if rows are less than columns
		std::cout << "psInv 11" << std::endl;
		int tmp = N;
		float* Xnew;
		Xnew = new float[M * N];
		transpose(Xnew, X, N, M);
